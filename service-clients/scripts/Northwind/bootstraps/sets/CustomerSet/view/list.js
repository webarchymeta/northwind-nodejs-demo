requirejs.config({
    ...
    paths: {
        ...
        queryTerms: 'service-clients/scripts/Northwind/customize/CustomerSet/querytokens',
        model: 'service-clients/scripts/Northwind/models/sets/Customer',
        ...
        listactions: 'service-clients/scripts/Northwind/controllers/sets/CustomerSet/view/list',
        actions: 'service-clients/scripts/Northwind/controllers/sets/CustomerSet/view/entity'
    }
});
...
requirejs(['knockout', 'pageManager', 'listactions', 'config', 'model'], function (ko, pMgr, opType, config, model) {
    ...
    pMgr.loadedControllerTable['service-clients/scripts/Northwind/controllers/sets/CustomerSet/view/list'] = opType;

    $(function () {
        pageMgr = pMgr;
        var filter = typeof subsetfilter === 'undefined' ? null : subsetfilter;
        pMgr.opContextType = opType;
        pMgr.categPath = 'CustomerSet';
        var op = new opType();
        currentClientConfig = config;
        op.initialize(filter).done(function (m) {
            pMgr.loadPage('CustomerSet/' + config.listViewEntryPart, m, op).done(function (page) {
                pMgr.opContext = 'service-clients/scripts/Northwind/controllers/sets/CustomerSet/view/list';
