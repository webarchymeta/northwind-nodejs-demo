requirejs.config({
    ...
    paths: {
        ...
        queryTerms: 'service-clients/scripts/Northwind/customize/CategorySet/querytokens',
        model: 'service-clients/scripts/Northwind/models/sets/Category',
        ...
        listactions: 'service-clients/scripts/Northwind/controllers/sets/CategorySet/view/list',
        actions: 'service-clients/scripts/Northwind/controllers/sets/CategorySet/view/entity'
    }
});
...
requirejs(['knockout', 'pageManager', 'listactions', 'config', 'model'], function (ko, pMgr, opType, config, model) {
    ...
    pMgr.loadedControllerTable['service-clients/scripts/Northwind/controllers/sets/CategorySet/view/list'] = opType;

    $(function () {
        pageMgr = pMgr;
        var filter = typeof subsetfilter === 'undefined' ? null : subsetfilter;
        pMgr.opContextType = opType;
        pMgr.categPath = 'CategorySet';
        currentClientConfig = config;
        var op = new opType();
        op.initialize(filter).done(function (m) {
            pMgr.loadPage('CategorySet/' + config.listViewEntryPart, m, op).done(function (page) {
                pMgr.opContext = 'service-clients/scripts/Northwind/controllers/sets/CategorySet/view/list';
