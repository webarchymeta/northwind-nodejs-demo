requirejs.config({
    ...
    paths: {
        ...
        queryTerms: 'service-clients/scripts/Northwind/customize/Order_DetailSet/querytokens',
        model: 'service-clients/scripts/Northwind/models/sets/Order_Detail',
        ...
        listactions: 'service-clients/scripts/Northwind/controllers/sets/Order_DetailSet/view/list',
        actions: 'service-clients/scripts/Northwind/controllers/sets/Order_DetailSet/view/entity'
    }
});
...
requirejs(['knockout', 'pageManager', 'listactions', 'config', 'model'], function (ko, pMgr, opType, config, model) {
    ...
    pMgr.loadedControllerTable['service-clients/scripts/Northwind/controllers/sets/Order_DetailSet/view/list'] = opType;

    $(function () {
        pageMgr = pMgr;
        var filter = typeof subsetfilter === 'undefined' ? null : subsetfilter;
        pMgr.opContextType = opType;
        pMgr.categPath = 'Order_DetailSet';
        currentClientConfig = config;
        var op = new opType();
        op.initialize(filter).done(function (m) {
            pMgr.loadPage('Order_DetailSet/' + config.listViewEntryPart, m, op).done(function (page) {
                pMgr.opContext = 'service-clients/scripts/Northwind/controllers/sets/Order_DetailSet/view/list';
