requirejs.config({
    ...
    paths: {
        ...
        queryTerms: 'service-clients/scripts/Northwind/customize/OrderSet/querytokens',
        model: 'service-clients/scripts/Northwind/models/sets/Order',
        ...
        listactions: 'service-clients/scripts/Northwind/controllers/sets/OrderSet/view/list',
        actions: 'service-clients/scripts/Northwind/controllers/sets/OrderSet/view/entity'
    }
});
...
requirejs(['knockout', 'pageManager', 'listactions', 'config', 'model'], function (ko, pMgr, opType, config, model) {
    ...
    pMgr.loadedControllerTable['service-clients/scripts/Northwind/controllers/sets/OrderSet/view/list'] = opType;

    $(function () {
        pageMgr = pMgr;
        var filter = typeof subsetfilter === 'undefined' ? null : subsetfilter;
        pMgr.opContextType = opType;
        pMgr.categPath = 'OrderSet';
        currentClientConfig = config;
        var op = new opType();
        op.initialize(filter).done(function (m) {
            pMgr.loadPage('OrderSet/' + config.listViewEntryPart, m, op).done(function (page) {
                pMgr.opContext = 'service-clients/scripts/Northwind/controllers/sets/OrderSet/view/list';
