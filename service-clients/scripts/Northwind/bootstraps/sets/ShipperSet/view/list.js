requirejs.config({
    ...
    paths: {
        ...
        queryTerms: 'service-clients/scripts/Northwind/customize/ShipperSet/querytokens',
        model: 'service-clients/scripts/Northwind/models/sets/Shipper',
        ...
        listactions: 'service-clients/scripts/Northwind/controllers/sets/ShipperSet/view/list',
        actions: 'service-clients/scripts/Northwind/controllers/sets/ShipperSet/view/entity'
    }
});
...
requirejs(['knockout', 'pageManager', 'listactions', 'config', 'model'], function (ko, pMgr, opType, config, model) {
    ...
    pMgr.loadedControllerTable['service-clients/scripts/Northwind/controllers/sets/ShipperSet/view/list'] = opType;

    $(function () {
        pageMgr = pMgr;
        var filter = typeof subsetfilter === 'undefined' ? null : subsetfilter;
        pMgr.opContextType = opType;
        pMgr.categPath = 'ShipperSet';
        var op = new opType();
        currentClientConfig = config;
        op.initialize(filter).done(function (m) {
            pMgr.loadPage('ShipperSet/' + config.listViewEntryPart, m, op).done(function (page) {
                pMgr.opContext = 'service-clients/scripts/Northwind/controllers/sets/ShipperSet/view/list';
