requirejs.config({
    ...
    paths: {
        ...
        queryTerms: 'service-clients/scripts/Northwind/customize/RegionSet/querytokens',
        model: 'service-clients/scripts/Northwind/models/sets/Region',
        ...
        listactions: 'service-clients/scripts/Northwind/controllers/sets/RegionSet/view/list',
        actions: 'service-clients/scripts/Northwind/controllers/sets/RegionSet/view/entity'
    }
});
...
requirejs(['knockout', 'pageManager', 'listactions', 'config', 'model'], function (ko, pMgr, opType, config, model) {
    ...
    pMgr.loadedControllerTable['service-clients/scripts/Northwind/controllers/sets/RegionSet/view/list'] = opType;

    $(function () {
        pageMgr = pMgr;
        var filter = typeof subsetfilter === 'undefined' ? null : subsetfilter;
        pMgr.opContextType = opType;
        pMgr.categPath = 'RegionSet';
        var op = new opType();
        currentClientConfig = config;
        op.initialize(filter).done(function (m) {
            pMgr.loadPage('RegionSet/' + config.listViewEntryPart, m, op).done(function (page) {
                pMgr.opContext = 'service-clients/scripts/Northwind/controllers/sets/RegionSet/view/list';
