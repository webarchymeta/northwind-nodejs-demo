...
requirejs(['knockout', 'pageManager', 'actions', 'config', 'model'], function (ko, pMgr, opType, config, model) {
    ...
    pMgr.loadedControllerTable['service-clients/scripts/Northwind/controllers/sets/CustomerDemographicSet/view/entity'] = opType;
    ...
    $(function () {
        ...
        var op = new opType();
        op.initialize(CustomerTypeID).done(function (m) {
            pMgr.loadPage(config.entityViewEntryPart, m).done(function (page) {
                pMgr.opContext = 'service-clients/scripts/Northwind/controllers/sets/view/CustomerDemographic';
