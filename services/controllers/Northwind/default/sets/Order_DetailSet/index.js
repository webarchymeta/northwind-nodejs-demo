...
var root = process.cwd(), path = require('path'),
    ...
    handler, api;
try {
    handler = require(path.join(__dirname, '../override/sets/Order_DetailSet'));
} catch (ex) {
    handler = null;
}
var isProxy = typeof config.inProcessDatabase === 'undefined' || config.inProcessDatabase.enable === false;
if (isProxy) {
    api = new (require(path.join(root, '../services/adapters/proxies/Northwind/api/sets/Order_DetailService')))();
} else {
    api = new (require(path.join(root, '../services/adapters/servers/Northwind/api/sets/Order_DetailService')))();
}
...
router.get('/edit/entity', function (req, res, next) {
    if (typeof router.filter !== 'object' || !router.filter.allowed || router.filter.readOnly) {
        ...
        res.render(router.vpath + '/edit/entity-shell.html', { title: 'Order_Detail', OrderID: parseInt(req.query.OrderID, 10), ProductID: parseInt(req.query.ProductID, 10) });
    }
});
...
router.get('/entity', function (req, res, next) {
    if (typeof router.filter !== 'object' || !router.filter.allowed) {
        ...
        res.render(router.vpath + '/view/entity-shell.html', { title: 'Order_Detail', OrderID: parseInt(req.query.OrderID, 10), ProductID: parseInt(req.query.ProductID, 10) });
    }
});
