...
var root = process.cwd(), path = require('path'),
    ...
    handler, api;
try {
    handler = require(path.join(__dirname, '../override/sets/SupplierSet'));
} catch (ex) {
    handler = null;
}
var isProxy = typeof config.inProcessDatabase === 'undefined' || config.inProcessDatabase.enable === false;
if (isProxy) {
    api = new (require(path.join(root, '../services/adapters/proxies/Northwind/api/sets/SupplierService')))();
} else {
    api = new (require(path.join(root, '../services/adapters/servers/Northwind/api/sets/SupplierService')))();
}

var is_rawdata_load = function(method) {
    var dmethods = [ 'LoadEntityHomePageRawData' ];
...
var is_delayedprop_update = function(method) {
    var dmethods = [ 'UpdateEntityHomePage' ];
    for (var i = 0; i < dmethods.length; i++) {
        if (dmethods[i] === method) {
            return true;
        }
...
router.get('/load-delayed-HomePage', function (req, res, next) {
    if (typeof router.filter !== 'object' || !router.filter.allowed) {
        res.writeHead(403);
        res.end();
    } else if ((router.filter.authenticate || router.filter['LoadEntityHomePage'].authenticate) && (typeof req.isAuthenticated !== 'function' || !req.isAuthenticated())) {
        res.writeHead(401);
        res.end();
        return;
    } else {
        ...
        var opts = { SupplierID: parseInt(req.query.SupplierID, 10) };
        api.LoadEntityHomePage(opts).then(function (data) {
            processRawData(req, res, data, router.filter['LoadEntityHomePage']);
        }).catch(function (err) {
            if (isProxy) {
                res.statusCode = err.httpCode === -1 ? 500 : err.httpCode;
                res.set('Content-Type', 'text/plain');
...
router.get('/edit/entity', function (req, res, next) {
    if (typeof router.filter !== 'object' || !router.filter.allowed || router.filter.readOnly) {
        ...
        res.render(router.vpath + '/edit/entity-shell.html', { title: 'Supplier', SupplierID: parseInt(req.query.SupplierID, 10) });
    }
});
...
router.get('/entity', function (req, res, next) {
    if (typeof router.filter !== 'object' || !router.filter.allowed) {
        ...
        res.render(router.vpath + '/view/entity-shell.html', { title: 'Supplier', SupplierID: parseInt(req.query.SupplierID, 10) });
    }
});
