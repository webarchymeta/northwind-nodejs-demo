...
var api = function () {
    ...
    self.baseUrl = config.NorthwindServiceBaseUrl + '/Services/DataService/Northwind/EmployeeTerritorySet.svc';
...
*/
    self.MaterializeTerritoryRef = function (input) {
        var __promise = new Q.Promise(function (resolve, reject, notify) {
            var opts = {
                url: self.baseUrl + '/MaterializeTerritoryRef',
                method: 'POST',
                json: {
                    cntx: config.mapCntx(input.cntx || cntx), 
                    entity: input.entity
            ...
            req(opts, function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    var data; 
                    if (typeof config.inProcessDatabase.wcfBackendService === 'undefined' || config.inProcessDatabase.wcfBackendService) {
                        data = body.MaterializeTerritoryRefResult;
                    } else {
                        data = body;
                    }
        ...
        var __promise = new Q.Promise(function (resolve, reject, notify) {
            ...
            req(opts, function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    ...
                    if ((typeof opts.json.cntx.allProperties === 'undefined' || !opts.json.cntx.allProperties) && (config.disablePropertyFiltering === 'undefined' || !config.disablePropertyFiltering) && typeof propFilters['EmployeeTerritory'] !== 'undefined') {
                        resolve(propFilters['EmployeeTerritory'].one(data, false));
                    } else {
                        resolve(data);
                    }
                } else {
