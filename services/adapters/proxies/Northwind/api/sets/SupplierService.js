...
var api = function () {
    ...
    self.baseUrl = config.NorthwindServiceBaseUrl + '/Services/DataService/Northwind/SupplierSet.svc';
...
*/
    self.LoadEntityHomePage = function (input) {
        var __promise = new B(function (resolve, reject) {
            var opts = {
                url: self.baseUrl + '/LoadEntityHomePage',
                method: 'POST',
                json: {
                    cntx: config.mapCntx(input.cntx || cntx),
                    _SupplierID: input._SupplierID || input.SupplierID
            ...
            req(opts, function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    var data; 
                    if (typeof config.inProcessDatabase.wcfBackendService === 'undefined' || config.inProcessDatabase.wcfBackendService) {
                        data = body.LoadEntityHomePageResult;
                    } else {
                        data = body;
                    }
                    resolve(data);
...
*/
    self.UpdateEntityHomePage = function (input) {
        var __promise = new B(function (resolve, reject) {
            var opts = {
                url: self.baseUrl + '/UpdateEntityHomePage',
                method: 'POST',
                json: {
                    cntx: config.mapCntx(input.cntx || cntx),
                    _SupplierID: input._SupplierID || input.SupplierID,
            ...
            req(opts, function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    var data; 
                    if (typeof config.inProcessDatabase.wcfBackendService === 'undefined' || config.inProcessDatabase.wcfBackendService) {
                        data = body.UpdateEntityHomePageResult;
                    } else {
                        data = body;
                    }
                    resolve(data);
